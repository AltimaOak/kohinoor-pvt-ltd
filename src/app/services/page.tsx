          ...
          {services.map((svc, index) => {
            ...
            return (
              <motion.div
                ...
                className={cn(
                  "bg-white border border-slate-200/50 rounded-[32px] p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-xl hover:border-sky-300 transition-all duration-500 relative overflow-hidden group hover:-translate-y-1.5 flex flex-col justify-between",
      ...
      <AnimatePresence>
        {isCampModalOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-navy-950/70 backdrop-blur-md overflow-y-auto p-4 md:p-8 flex items-start justify-center"
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl bg-slate-50 border border-white/60 rounded-[36px] shadow-2xl p-8 md:p-12 my-auto min-h-[500px] flex flex-col gap-10"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsCampModalOpen(false)}
                className="absolute top-6 right-6 p-2 rounded-full text-slate-400 hover:text-navy-900 hover:bg-slate-100 transition-colors focus:outline-none cursor-pointer z-50"
              >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
                  ...
                  {doctors.map((doctor, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={cn(
                          "bg-white border rounded-3xl p-6 transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-6 cursor-pointer relative overflow-hidden group/doc",
                          isSelectedForQr 
                      ...
                      >
                        ...
                        <div className="flex items-start gap-4">
                          <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center border shrink-0 transition-all duration-300", 
                          ...
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                              <h4 className="text-base font-extrabold text-navy-900 tracking-tight leading-tight">
                                {doctor.name}
                              </h4>
                              {isSelectedForQr && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-sky-100 text-sky-700 uppercase tracking-wide">
                                  Active QR
                                </span>
                              )}
                            </div>
                            <span className="text-xs text-sky-600 font-bold tracking-wide">
                              {doctor.specialty}
                            </span>
                            
                            <div className="flex flex-col gap-1 mt-1 text-[11px] text-slate-500">
                              <div className="flex items-center gap-1.5">
                                <LucideIcons.CalendarRange className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                                <span>Visiting: <span className="font-semibold text-slate-700">{doctor.schedule}</span></span>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <LucideIcons.Phone className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                                <span className="font-mono text-slate-700">{doctor.phone}</span>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <LucideIcons.Mail className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        ...
                        <button
                          ...
                          className={cn(
                            "sm:self-center px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 shrink-0 shadow-sm flex items-center justify-center gap-1.5 cursor-pointer",
                            isSelectedForQr
                ...
                {/* QR Card */}
                <div className="bg-gradient-to-br from-navy-950 to-slate-900 rounded-[32px] p-6 text-white shadow-xl flex flex-col justify-between items-center text-center border border-slate-800 relative overflow-hidden min-h-[380px]">
